...
public:
	void Run(){
		...
		for(It = wxGetApp().m_marked_list->list().begin(); It != wxGetApp().m_marked_list->list().end(); It++){
			...
			if(last)
			{
				HDimension* dimension = new HDimension(gp_Trsf(), last->m_p, obj->m_p, gp_Pnt(), TwoPointsDimensionMode, PythagoreanDimensionTextMode, DimensionUnitsGlobal, &wxGetApp().dimension_color);
...
public:
	void Run(){
		...
		HDimension* dimension = new HDimension(gp_Trsf(), circle->C->m_p, circle->C->m_p.XYZ() + circle->m_radius * gp_XYZ(1,0,0), gp_Pnt(), TwoPointsDimensionMode, PythagoreanDimensionTextMode, DimensionUnitsGlobal, &wxGetApp().dimension_color);
...
public:
	void Run(){
		...
		HDimension* dimension = new HDimension(gp_Trsf(), arc->C->m_p, arc->C->m_p.XYZ() + arc->m_radius * gp_XYZ(1,0,0), gp_Pnt(), TwoPointsDimensionMode, PythagoreanDimensionTextMode, DimensionUnitsGlobal, &wxGetApp().dimension_color);
